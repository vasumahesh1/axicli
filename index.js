...
var appendMissingConfigs = function(filePath, configCli) {
  ...
  for (var i = configCli.length - 1; i >= 0; i--) {
    if (fileData.indexOf(configCli[i]) == -1) {
      fs.appendFileSync(filePath, configCli[i] + LINE_ENDING);
    }
  };
};
...
var ServerGen = function() {
  var _installServer = function(serverObject) {
    var setupVariableSsh = function(serverObject) {
      ...
      serverConfig += LINE_ENDING + '# [Auto Generated] SERVER TEMPLATE : ' + serverObject.name + LINE_ENDING;
      serverConfig += serverTemplate + LINE_ENDING + LINE_ENDING;
    };

    var easySsh = function(serverObject) {
      serverConfig += LINE_ENDING + '# [Auto Generated] SSH : ' + serverObject.name + LINE_ENDING;
      serverConfig += 'alias ssh-' + serverObject.name + '=\'ssh_' + serverObject.name + ' $@\'' + LINE_ENDING;
      serverConfig += 'alias ssh-root-' + serverObject.name + '=\'ssh_' + serverObject.name + ' --user root $@\'' + LINE_ENDING;
    };

    var easyCopyFrom = function(serverObject) {
      serverConfig += LINE_ENDING + '#[Auto Generated] Copy From : ' + serverObject.name + LINE_ENDING;
      serverConfig += 'alias copy-from-' + serverObject.name + '=\'copy_from_server ' + currentConfig.user + ' ' + serverObject.ip + ' ' + '/home/' + currentConfig.user + '/' + ' $@' + '\'' + LINE_ENDING;
      serverConfig += 'alias copy-from-root-' + serverObject.name + '=\'copy_from_server root ' + serverObject.ip + ' /root/ $@\'' + LINE_ENDING;
    };

    var easyCopyTo = function(serverObject) {
      serverConfig += LINE_ENDING + '# [Auto Generated] Copy To : ' + serverObject.name + LINE_ENDING;
      serverConfig += 'alias copy-to-' + serverObject.name + '=\'copy_to_server ' + currentConfig.user + ' ' + serverObject.ip + ' ' + '/home/' + currentConfig.user + '/' + ' $@' + '\'' + LINE_ENDING;
      serverConfig += 'alias copy-to-root-' + serverObject.name + '=\'copy_to_server root ' + serverObject.ip + ' /root/ $@\'' + LINE_ENDING;
    };
  ...
  var _installConfig = function(configType, callback) {
    if (configType == 'server' && serverConfig && serverConfig != '') {
      fs.exists(AXI_SHELL_CONFIG, function(exists) {
        ...
        var mainTemplate = baseTemplate + LINE_ENDING + serverConfig;
